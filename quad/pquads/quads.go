...
//go:generate protoc --proto_path=. --go_out=. --go_opt=module=github.com/ducesoft/cayley/quad/pquads quads.proto
...
func MakeValue(qv quad.Value) *Value {
	...
	case quad.IRI:
		return &Value{Value: &Value_Iri{string(v)}}
	case quad.BNode:
		return &Value{Value: &Value_Bnode{string(v)}}
	case quad.TypedString:
		return &Value{Value: &Value_TypedStr{&Value_TypedString{
			Value: string(v.Value),
			Type:  string(v.Type),
		}}}
	case quad.LangString:
		return &Value{Value: &Value_LangStr{&Value_LangString{
			Value: string(v.Value),
			Lang:  v.Lang,
		}}}
	case quad.Int:
		return &Value{Value: &Value_Int{int64(v)}}
	case quad.Float:
		return &Value{Value: &Value_Float{float64(v)}}
	case quad.Bool:
		return &Value{Value: &Value_Boolean{bool(v)}}
	case quad.Time:
		t := time.Time(v)
		seconds := t.Unix()
		nanos := int32(t.Sub(time.Unix(seconds, 0)))
		return &Value{Value: &Value_Time{&Value_Timestamp{
			Seconds: seconds,
			Nanos:   nanos,
		}}}
	default:
